<?php include('header.tpl.php') ?>
    ...
    <form class="form-horizontal custom" method="POST">
      ...
      <div class="form-group<?php print !empty($titleError)?' text-danger error':'' ?>">
          ...
          <div class="col-xs-9">
              <input name="title" type="text" class="form-control" id="title" placeholder="Title" value='<?php print !empty($title)?$title:'';?>'>
              <!-- autofocus -->
      ...
      <div class="form-group<?php print !empty($option_timestartError)?' text-danger error':'' ?>">
          <label class="control-label col-xs-3" for="option_timestart">Start to show after time (current time as default):</label>
          <div class="col-xs-2">
          ...
          <div class="col-xs-1">
            <select name="option_timestart_hours" id="hours" class="form-control">
              <?php for ($h=0; $h <= 23; $h++): ?>
                <option value="<?php printf("%'02d", $h) ?>" <?php print ($option_timestart_hours == $h)?' selected':'' ?>>
                  <?php printf("%'02d", $h) ?>
                </option>
              <?php endfor; ?>
            </select> 
      ...
      <div class="form-group<?php print !empty($option_timeendError)?' text-danger error':'' ?>">
          <label class="control-label col-xs-3" for="option_timeend">Show before this time (not required):</label>
          <div class="col-xs-2">
          ...
          <div class="col-xs-1">
            <select name="option_timeend_hours" id="hours" class="form-control">
              <?php for ($h=0; $h <= 23; $h++): ?>
                <option value="<?php printf("%'02d", $h) ?>" <?php print ($option_timeend_hours == $h)?' selected':'' ?>>
                  <?php printf("%'02d", $h) ?>
                </option>
              <?php endfor; ?>
            </select> 
